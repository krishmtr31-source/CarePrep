...
export const CarePrepAppHeader: React.FC<CarePrepAppHeaderProps> = ({
  ...
  useEffect(() => {
    ...
    llmGateway.checkLiveConnection().then(() => {
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {providerStatus.isGeminiLive ? (
                  <span className="hidden md:inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-300 animate-in fade-in">
                    <Sparkles className="w-3 h-3 text-emerald-600 animate-pulse" />
                    ✨ Gemini 2.5 Flash • Connected
                  </span>
                ) : (
                  <span className="hidden md:inline-flex items-center gap-1 text-[10px] font-semibold text-amber-800 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">
