...
async function runTests() {
  ...
  assert(validated.data.missingInformation?.includes('Radiation') === true, 'Missing information list must be preserved');
  console.log('[PASS] Test 3: 3. Structured Gemini JSON Output Validation');
  passed++;

  // Test 4: English Natural Language Interpretation
  ...
  console.log('[PASS] Test 4: 4. English Patient Natural Language Interpretation (Headache, 2 days)');
  passed++;

  // Test 5: Hindi Natural Language Interpretation
  ...
  console.log('[PASS] Test 5: 5. Hindi Patient Natural Language Interpretation (पेट में दर्द -> Abdominal Pain)');
  passed++;

  // Test 6: Tamil Natural Language Interpretation
  ...
  console.log('[PASS] Test 6: 6. Tamil Patient Natural Language Interpretation (தலைவலி -> Headache)');
  passed++;

  // Test 7: Hinglish / Mixed Language Interpretation
  ...
  console.log('[PASS] Test 7: 7. Hinglish / Mixed Language Normalization');
  passed++;

  // Test 8: SOCRATES Structured Extraction
  ...
  console.log('[PASS] Test 8: 8. SOCRATES Structured Extraction (Site: ' + validated.data.location + ', Onset: ' + validated.data.duration + ', Character: ' + validated.data.character + ')');
  passed++;

  // Test 9: Missing Field Detection
  ...
  console.log('[PASS] Test 9: 9. Missing Field Detection: ' + validated.data.missingInformation?.join(', '));
  passed++;

  // Test 10: Next Question Generation from Allowed Framework
  assert(typeof validated.data.nextQuestion === 'string' && validated.data.nextQuestion.length > 0, 'Next question must be generated');
  console.log('[PASS] Test 10: 10. Adaptive Next Question Suggestion: "' + validated.data.nextQuestion + '"');
  passed++;

  // Test 11: Original Patient Wording Preserved Exactly
  assert(validated.data.originalText === 'Mujhe 3 din se pet mein bahut tej dard hai', 'Verbatim patient text must be preserved');
  console.log('[PASS] Test 11: 11. Verbatim Original Patient Statement Preserved for Audit');
  passed++;

  // Test 12: Gemini Failure Automatic Fallback to Deterministic NLP
  ...
  console.log('[PASS] Test 12: 12. Gemini Failure Automatic Fallback to Deterministic NLP');
  passed++;

  // Test 13: Malformed JSON Fallback Resilience
  ...
  console.log('[PASS] Test 13: 13. Malformed Gemini Response Grounding Resilience');
  passed++;

  // Test 14: Prompt Injection Defense in Patient Input
  ...
  console.log('[PASS] Test 14: 14. Prompt Injection Defense Intercepts System Commands');
  passed++;

  // Test 15: Session Request Limit Enforcement
  ...
  console.log('[PASS] Test 15: 15. Session Safety Request Limit Guard (Max 20 Requests)');
  passed++;

  // Test 16: Red-Flag Emergency Hierarchy (RED Always Wins Over LLM)
  ...
  console.log('[PASS] Test 16: 16. Safety Hierarchy: Deterministic Red-Flag ALWAYS Overrides LLM (RED Wins)');
  passed++;

  // Test 17: Gemini Cannot Bypass Safety Controller
  ...
  console.log('[PASS] Test 17: 17. Gemini Cannot Bypass Safety Controller Routing (Status: ' + orchRes.orchestratorContext.safetyStatus + ')');
  passed++;

  // Test 18: AYUSH Patient-Reported Separation
  ...
  console.log('[PASS] Test 18: 18. AYUSH Mode Separation (Patient Reported vs Clinician Nadi Pariksha)');
  passed++;

  // Test 19: Evidence Provenance Preserved Across Multi-Modal Inputs
  ...
  console.log('[PASS] Test 19: 19. Multi-Modal Evidence Provenance Traceability (Source: ' + orchRes.orchestratorContext.evidenceTrail[0].sourceType + ')');
  passed++;

  // Test 20: UI Dynamic Provider Status Correctness
  ...
  console.log('[PASS] Test 20: 20. UI Provider Dynamic Status Labeling: "' + status.statusLabel + '"');
  passed++;

  console.log(`\nTOTAL PHASE 7 GEMINI INTEGRATION TESTS: ${passed} / 20 PASSED.`);
}
